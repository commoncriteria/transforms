...
<tr><td><span id='bibCC'> [CC] </span></td><td>Common Criteria for Information Technology Security Evaluation - <ul>
  ...
  <xsl:template match="cc:selectables">[<b>selection</b>
	  <xsl:if test="@onlyone or @only-one or @choose-one-of"><b>, choose one of</b></xsl:if><xsl:text>: </xsl:text>
			...
			<xsl:for-each select="cc:mgmt-func-def">
				...
				<!--</tr> -->
					...
					<xsl:when test="./cc:app-note">
						<tr><td colspan="3"><b>Application Note: </b><xsl:apply-templates select="./cc:app-note"/></td></tr> 
					</xsl:when>
					<xsl:otherwise>
